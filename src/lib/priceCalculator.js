export function calculatePrice(wizard, products, priceRules) {
  ...
  if (stovePosition) { const p=products.find(p=>p.category==="tehnika"&&p.name_lv.includes("Indukcijas")); bd.stove=p?.base_price||0; total+=bd.stove; }
  if (ovenPosition)  { const p=products.find(p=>p.category==="tehnika"&&p.name_lv.includes("standarta")); bd.oven=p?.base_price||0; total+=bd.oven; }
  if (fridgePosition){ const kw=fridgeType==="iebuvejams"?"Iebūvējams":"Atsevišķs"; const p=products.find(p=>p.category==="tehnika"&&p.name_lv.includes(kw)); bd.fridge=p?.base_price||0; total+=bd.fridge; }
